...
export default {
    ...
    methods: {
        ...
        //分页切换
        handlePage(page){
            this.$router.push({ path: '/apps/' + this.appsType + '-' + page});
        }
    },
    asyncData ({params}) {
        var str = params.id;
        ...
        var appsType = parseInt(type);
        var current = parseInt(last);
        return axios.all([
            axios.get('http://h5.7guoyouxi.com/html/index/game_more_data?type=' + appsType),
        ...
        .then(axios.spread(function (game_more_data, hot_game) {
            return {
                appsType: appsType,
                current: current,
                appsData: game_more_data.data.data.slice(10 * (current - 1), 10 * current),
